...
describe('ConflictResolver', () => {
  ...
  describe('Conflict Detection', () => {
    it('should detect conflicts', () => {
      expect(conflictResolver.detectConflict({ id: '1', value: 'local' }, { id: '1', value: 'server' })).toBe(true);
    });

    it('should not detect conflicts when states match', () => {
      expect(conflictResolver.detectConflict({ id: '1', value: 'same' }, { id: '1', value: 'same' })).toBe(false);
    });

    it('should detect conflicts with different keys', () => {
      expect(conflictResolver.detectConflict({ id: '1', value: 'a' }, { id: '1', value: 'a', extra: 'field' })).toBe(true);
    });
  });
